...
export default class CreateEdit extends Component {
    ...
    onSubmit = (e) => {
        ...
        const { _id, pv, cantidad, prefijo, costo, plan, vendidas, total_cobrado, comision, ganancia_neta, folio_error, isEditing } = this.state

        if(!pv) {
            return toast.warn("Completa el campo de nombre");
        }

        axios.post(process.env.REACT_APP_SERVER_IP + `api/fichas${isEditing ? "/editar" : "/nuevo"}`, { _id, pv: pv._id, cantidad, prefijo, costo, plan, vendidas, total_cobrado, comision, ganancia_neta, folio_error })
            .then(res => {
    ...
    render() {
        ...
        const { pv, cantidad, prefijo, costo, plan, vendidas, total_cobrado, comision, ganancia_neta, folio_error, isEditing, notFound } = this.state
            ...
            <form onSubmit={this.onSubmit}>
              ...
              <div className='form-content'>
                <div className="row">
                  ...
                  <div className="col-sm-2">
                    <div className="form-group mb-4">
                      <input onChange={this.onChange} value={folio_error || ""} type="text" className="form-control form-control frm_field" placeholder="Folio" name="folio_error" />
                      <small className="form-text text-muted">Folio del error</small>
                    </div>
                  </div>
                </div>
              </div>
